...
export async function generateMetadata() {
  const lld = await getContent("location");
  return pageMetadata({ ...lld.seo, path: PATH, image: lld.hero.image.src });
}

export default async function LldPage() {
  const [lld, { keyFigures }] = await Promise.all([getContent("location"), getContent("site")]);
