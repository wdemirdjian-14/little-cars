...
export async function generateMetadata() {
  const little = await getContent("little");
  return pageMetadata({ ...little.seo, path: PATH, image: little.hero.image.src });
}

export default async function LittlePage() {
  const [little, { keyFigures }] = await Promise.all([getContent("little"), getContent("site")]);
