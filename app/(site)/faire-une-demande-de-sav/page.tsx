...
export async function generateMetadata() {
  const savRequest = await getContent("demande-sav");
  return pageMetadata({ ...savRequest.seo, path: PATH, image: savRequest.hero.image.src });
}

export default async function SavPage() {
  const [savRequest, { company }] = await Promise.all([getContent("demande-sav"), getContent("site")]);
  const crumbs = [
